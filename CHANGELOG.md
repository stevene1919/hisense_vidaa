...
### Added
- **OPT-IN PICTURE & SOUND CONTROLS**: Picture calibration sliders (`number.{tv}_backlight`, `number.{tv}_brightness`, `number.{tv}_contrast`) and mode selects (`select.{tv}_picture_mode`, `select.{tv}_sound_mode`) are now opt-in via **Configure -> 🎨 Picture & Sound Controls** (default: disabled). Prevents phantom controls from cluttering Home Assistant on TVs lacking MQTT picture/sound menu support.
- **AUTOMATIC ENTITY REGISTRY CLEANUP**: Disabled picture and sound entities are automatically purged from Home Assistant's Entity Registry on setup/options update rather than lingering as unavailable or orphaned entities.
